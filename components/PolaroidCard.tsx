...
const PolaroidCard: React.FC<PolaroidCardProps> = ({ imageUrl, caption, status, error, onRegenerate, onDownload }) => {
    // If the card is mounted with a 'done' status and an image,
    // it means we are viewing an already generated image. No animation needed.
    const [isDeveloped, setIsDeveloped] = useState(status === 'done' && !!imageUrl);
    const [isImageLoaded, setIsImageLoaded] = useState(status === 'done' && !!imageUrl);

    // This effect handles the state reset when an image is being regenerated.
    useEffect(() => {
        if (status === 'pending') {
    ...
    // This effect triggers the "developing" animation when an image loads for the first time.
    useEffect(() => {
        // Trigger animation only if the image has loaded but hasn't been marked as "developed" yet.
        if (isImageLoaded && !isDeveloped) {
            const timer = setTimeout(() => {
                setIsDeveloped(true);
            }, 200); // A short delay to simulate the developing process starting.
            return () => clearTimeout(timer);
        }
    }, [isImageLoaded, isDeveloped]);

    return (
         <motion.div 
            className="bg-neutral-100 p-4 pb-16 flex flex-col items-center justify-start aspect-[3/4] w-80 max-w-full rounded-md shadow-lg relative transform transition-transform duration-300 hover:scale-105"
        ...
        >
            <div className="w-full bg-neutral-900 shadow-inner flex-grow relative overflow-hidden group">
                ...
                {status === 'done' && imageUrl && (
                    <>
                        ...
                        <div
                            className={`absolute inset-0 z-10 bg-[#3a322c] transition-opacity duration-[3500ms] ease-out pointer-events-none ${
            ...
            <div className="absolute bottom-4 left-4 right-4 text-center px-2">
                <p className={`font-medium text-lg truncate ${status === 'done' && imageUrl ? 'text-black' : 'text-neutral-800'}`}>
                    {caption}
                </p>
            </div>
        </motion.div>
